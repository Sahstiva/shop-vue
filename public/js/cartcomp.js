Vue.component("cartlist", {
  ...
  changeProduct(item, change) {
      let find = this.$parent.$refs.products.cart.find((el) => el.id === item.id);
      if (find) {
        this.$parent
          .putJson(`${this.$parent.API}${this.$parent.$refs.products.cartUrl}/${find.id}`, { quantity: find.quantity + parseInt(change) })
          .then((data) => {
            if (data.result === 1) {
    ...
    removeProduct(item) {
      let find = this.$parent.$refs.products.cart.find((el) => el.id === item.id);
      this.$parent
        .deleteJson(`${this.$parent.API}${this.$parent.$refs.products.cartUrl}/${find.id}`, find)
        .then((data) => {
          if (data.result === 1)
            this.$parent.$refs.products.cart = this.$parent.$refs.products.cart.filter((el) => el.id != item.id);
            this.$parent.$refs.products.calcQuantity();
